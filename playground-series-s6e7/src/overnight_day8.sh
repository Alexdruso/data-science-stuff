...
gpu_chain() {
  # 1) _r2 rebuild remainder (mult 0.5; xgb s42 already done, skip-if-exists resumes)
  S6E7_TAG_PREFIX=_r2 S6E7_REPAIR=1 S6E7_REPAIR_MULT=0.5 \
    bash src/run_breadth.sh xgboost catboost
  # 2) zoo Z4 DANN + mask-consistency (trains on RAW matrix by design — no S6E7_REPAIR)
  S6E7_SEEDS=42 S6E7_RUN_TAG=_r_s42 "$PY" src/train_dann.py \
    > results/log_zoo_dann.txt 2>&1
  # 3) zoo Z3 TabM retry (priced out of the day at >25 min/fold; free overnight)
