...
# Same namespacing as run_breadth.sh: S6E7_TAG_PREFIX=_r combines the repaired
# per-seed arrays (oof_<model>_r_s<seed>.npy) and suffixes every output, so the
# repaired and unrepaired breadth candidates never overwrite each other.
PREFIX = os.environ.get("S6E7_TAG_PREFIX", "")
...
def combine_seeds(model: str, y: np.ndarray) -> str | None:
    ...
    oofs, tests, used = [], [], []
    for s in SEEDS:
        op = RESULTS_DIR / f"oof_{model}{PREFIX}_s{s}.npy"
        tp = RESULTS_DIR / f"test_{model}{PREFIX}_s{s}.npy"
...
def main() -> None:
    ...
    np.save(RESULTS_DIR / f"oof_ensemble{PREFIX}_breadth.npy", blended_oof)
    np.save(RESULTS_DIR / f"test_ensemble{PREFIX}_breadth.npy", blended_test)
    with (RESULTS_DIR / f"decision_weights_ensemble{PREFIX}_breadth.json").open("w") as f:
    ...
    out = SUBMISSIONS_DIR / f"final2_breadth{PREFIX}.csv"
