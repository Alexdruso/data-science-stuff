...
def objective(trial):
    ...
    for train_idx, val_idx in kf.split(ranking_train["id"].unique()):
        ...
        # Prepare features
        X_train_fold = preprocessor.fit_transform(
            train_fold[numeric_features + categorical_features]
        )
        X_val_fold = preprocessor.transform(
            val_fold[numeric_features + categorical_features]
        )
...
X_train = preprocessor.fit_transform(
    ranking_train[numeric_features + categorical_features]
)
...
best_model.fit(X_train, ranking_train["is_target"], group=train_groups, verbose=False)
...
submission["Fertilizer Name"] = [
    " ".join(le.inverse_transform(indices)) for indices in top_3_indices
]
...
train_map = label_ranking_average_precision_score(
    ranking_train["is_target"].values.reshape(-1, len(fertilizers)), train_pred_reshaped
)
