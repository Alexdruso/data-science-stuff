...
print(
    "Logistic regression baseline submission saved to data/logistic_baseline_submission.csv"
)
...
train_predictions = [
    " ".join(le.inverse_transform(indices)) for indices in train_top_3_indices
]
train_accuracy = sum(
    1 for true, pred in zip(y_train, train_predictions) if true in pred.split()
) / len(y_train)
